...
public class CoreDictionary
{
    ...
    {   
        ...
        {   
//        	System.out.println(path + "加载成功，" + trie.size() + "个词条，耗时" + (System.currentTimeMillis() - start) + "ms");
...
//        BufferedReader br = null;
        List<String> retainedAttributes=Arrays.asList("未##串 x 130296",
    		...
    		"未##时 t 757118");//统一设定7类未登录词标志属性
        try
        {
//            br = new BufferedReader(new InputStreamReader(IOUtil.newInputStream(path), "UTF-8"));
            List<String> pathParts=Arrays.asList(path.split("(\\\\|/)"));
            List<String> lines=Files.readAllLines(FileSystems.getDefault().getPath(path.replaceAll("(/|\\\\)Core\\S+",""), pathParts.get(pathParts.size()-1)),StandardCharsets.UTF_8);
            lines.addAll(retainedAttributes);//加入7类未登录词标志属性
            int MAX_FREQUENCY = 0;
            long start = System.currentTimeMillis();
            ...
            {   
            	if(line!=null){
	            	try{
		                ...
		                if(param[0].contains("未##"))System.out.println("Specials ......"+param[0]+","+param[1]);
...
//            System.out.println("核心词典读入词条" + map.size() + " 全部频次" + MAX_FREQUENCY + "，耗时" + (System.currentTimeMillis() - start) + "ms");
            lines=null;

            trie.build(map);
            logger.info("核心词典加载成功:" + trie.size() + "个词条，下面将写入缓存……");
//            System.out.println("核心词典加载成功:" + trie.size() + "个词条，下面将写入缓存……");
    ...
    /**
     * 从磁盘加载双数组，即是前一次成功加载过的txt字典产生的bin文件
     *
     * @param path
     * @return
     */
    ...
    {   
    	File datafbin=new File(path + Predefine.BIN_EXT);
    	if(datafbin.length()<1000)return false;//如果bin文件过小，说明是生成失败或有意改小以进行词典重新加载
        try
        {
